...
class BindingRequestCacheRegistry:
    ...
    def invalidate_pending_binding_requests(self, filter_time: datetime):
        ...
        self._registry = {
            ...
            if filter_time - v.timestamp < MAX_BINDING_REQUEST_TIMEOUT
        }
    ...
    def cache_message(self, msg: stun.Message, dst: tuple[str, int]):
...
class SelectorProtocol(Protocol):
    ...
    def send_ping_stun_message(self, pair: CandidatePair, conn: MuxConnProtocol): ...
...
class ControllingSelector:
    ...
    def _stun_nominate_pair(self, pair: CandidatePair, conn: MuxConnProtocol):
        msg = stun.Message(
        ...
        self._local_binding_cache.cache_message(
            msg,
            (
    ...
    def on_binding_success(
        ...
        if pair.state == CandidatePairState.SUCCEEDED and self._nominated_pair is None:
            self._stun_nominate_pair(pair, conn)
...
class ControlledSelector:
    ...
    def on_binding_success(
        ...
        self.send_ping_stun_message(pair, conn)
...
class CandidatePairController:
    ...
    def _on_stun_binding_request(self, pkt: Packet, msg: stun.Message):
        # print("On stun binding request", msg)
        self._selector.on_binding_success(self._pair, self._conn, msg)
        # print("On stun binding request", self._pair.state)

    def _on_stun_success_response(self, pkt: Packet, msg: stun.Message):
        self._selector.on_success_response(self._pair, self._conn, msg, pkt.source)
                ...
                case stun.Method.Binding:
                    match msg.message_type.message_class:
                        case stun.MessageClass.Request:
                            ...
                            self._on_stun_binding_request(pkt, msg)
                        case stun.MessageClass.SuccessResponse:
                            msg = stun_message_parse_attrs(
                                pkt, msg, self._pair.remote_pwd
                            )
                            self._on_stun_success_response(pkt, msg)
    ...
    def ping_remote_candidate(self):
        """
        ...
        self._selector.send_ping_stun_message(self._pair, self._conn)
...
class Agent:
    ...
    def connect(self, controlling: bool, remote_ufrag: str, remote_pwd: str):
        ...
        for pair in self._pair_registry.get_pair_list():
            ...
            pair_controller.ping_remote_candidate()
